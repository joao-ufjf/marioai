...
//                        show(cur_char);
 ...
 if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ -1 ] != 20 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ if( landscape[11+ 0 ][11+ 1 ] != 20 ){ if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } if( enemies[11+ 0 ][11+ -1 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_JUMP ] = false ;
 } } } } } 	
	//Begin of BGPAgent
	return action;
    }
