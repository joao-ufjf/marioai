...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_SPEED ] = true ;
 ...
 } } } if( enemies[11+ 2 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ -1 ][11+ 2 ] != Sprite.KIND_BULLET_BILL ){ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = false ;
 ...
 action[ Mario.KEY_LEFT ] = false ;
 } }else{ if( landscape[11+ 0 ][11+ 1 ] != 21 ){ action[ Mario.KEY_DOWN ] = false ;
