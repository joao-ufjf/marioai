 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent467 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ -1 ][11+ 0 ] != 1 ){ if( landscape[11+ -1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_SPEED ] = false ;
 }else{ if( landscape[11+ -1 ][11+ -1 ] != 1 ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } if( enemies[11+ 0 ][11+ 0 ] != 1 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = false ;
