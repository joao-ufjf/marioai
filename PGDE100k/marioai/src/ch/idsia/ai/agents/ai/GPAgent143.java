...
//                        show(cur_char);
 ...
 if( observation.isMarioOnGround() ){ if( landscape[11+ 2 ][11+ 1 ] != 20 ){ if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_DOWN ] = false ;
 } } }else{ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_SPEED ] = true ;
 } if( landscape[11+ -2 ][11+ -2 ] != 20 ){ action[ Mario.KEY_LEFT ] = false ;
 } } action[ Mario.KEY_LEFT ] = true ;
 } }else{ action[ Mario.KEY_JUMP ] = true ;
 if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 2 ] != -11 ){ if( landscape[11+ 2 ][11+ 0 ] != 16 ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } } if( enemies[11+ 0 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ action[ Mario.KEY_JUMP ] = true ;
 action[ Mario.KEY_RIGHT ] = true ;
 }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
 } } }else{ if( observation.isMarioOnGround() ){ if( landscape[11+ 2 ][11+ 0 ] != 20 ){ if( enemies[11+ 1 ][11+ 1 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_RIGHT ] = true ;
 } } } } if( landscape[11+ -2 ][11+ 1 ] != -11 ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_SPEED ] = true ;
