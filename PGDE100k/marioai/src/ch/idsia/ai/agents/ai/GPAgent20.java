...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 } } }else{ if( observation.mayMarioJump() ){ if( landscape[11+ -2 ][11+ 1 ] != 21 ){ if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 0 ] != 16 ){ if( landscape[11+ 0 ][11+ -2 ] != -10 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } } } } }else{ if( landscape[11+ 1 ][11+ 2 ] != -11 ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_SPIKY ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ action[ Mario.KEY_DOWN ] = true ;
 } } if( observation.isMarioOnGround() ){ if( landscape[11+ -2 ][11+ 0 ] != 0 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } } } } 	
	//Begin of BGPAgent
