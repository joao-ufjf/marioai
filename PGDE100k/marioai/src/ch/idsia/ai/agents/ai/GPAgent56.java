...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ 1 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
 if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ 0 ] != 16 ){ action[ Mario.KEY_JUMP ] = false ;
 if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 } } }else{ if( enemies[11+ 0 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = false ;
 } action[ Mario.KEY_RIGHT ] = true ;
 } } }else{ if( enemies[11+ 0 ][11+ 2 ] != Sprite.KIND_SPIKY_WINGED ){ if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 2 ] != -10 ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_SPEED ] = false ;
 action[ Mario.KEY_SPEED ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ if( landscape[11+ -2 ][11+ 1 ] != 20 ){ action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_LEFT ] = true ;
 } } } }else{ if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = false ;
 } if( landscape[11+ 0 ][11+ 0 ] != 20 ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = false ;
