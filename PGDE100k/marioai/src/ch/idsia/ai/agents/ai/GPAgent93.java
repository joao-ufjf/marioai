...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( enemies[11+ 0 ][11+ 2 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( enemies[11+ -2 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 1 ] != 16 ){ if( landscape[11+ 1 ][11+ -1 ] != 16 ){ action[ Mario.KEY_DOWN ] = false ;
 } } } } } 	
	//Begin of BGPAgent
	return action;
    }
