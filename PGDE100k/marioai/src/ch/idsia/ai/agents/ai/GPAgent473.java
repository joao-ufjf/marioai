 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent473 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ -1 ][11+ 0 ] != 1 ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
 ...
 } } }else{ if( enemies[11+ -1 ][11+ 1 ] != 1 ){ if( enemies[11+ -1 ][11+ -1 ] != 1 ){ action[ Mario.KEY_LEFT ] = false ;
