...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ 0 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_RIGHT ] = false ;
 if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } } }else{ if( landscape[11+ 0 ][11+ 1 ] != 21 ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } } if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 0 ] != 0 ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } action[ Mario.KEY_JUMP ] = false ;
 if( observation.isMarioOnGround() ){ if( landscape[11+ 0 ][11+ 1 ] != 21 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 } } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } if( landscape[11+ 1 ][11+ -1 ] != 16 ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } } } 	
	//Begin of BGPAgent
	return action;
