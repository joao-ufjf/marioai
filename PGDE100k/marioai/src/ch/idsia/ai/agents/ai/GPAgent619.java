 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent619 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 }else{ if( enemies[11+ 1 ][11+ -1 ] != 1 ){ if( landscape[11+ -1 ][11+ -1 ] != 1 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } }else{ if( enemies[11+ -1 ][11+ -1 ] != 1 ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } } if( enemies[11+ 1 ][11+ 0 ] != 1 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
