...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 2 ] != 16 ){ action[ Mario.KEY_JUMP ] = false ;
 } } if( landscape[11+ -2 ][11+ -2 ] != 0 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_JUMP ] = true ;
 } action[ Mario.KEY_LEFT ] = true ;
 }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = true ;
 } action[ Mario.KEY_DOWN ] = false ;
 } }else{ if( landscape[11+ 0 ][11+ -1 ] != 0 ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 if( landscape[11+ 1 ][11+ -1 ] != 20 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = true ;
