...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( landscape[11+ 1 ][11+ 1 ] != -11 ){ if( enemies[11+ 1 ][11+ 1 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_JUMP ] = false ;
 } if( enemies[11+ 0 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ if( enemies[11+ 2 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
