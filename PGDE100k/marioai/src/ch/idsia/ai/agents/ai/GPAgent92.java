...
//                        show(cur_char);
 ...
 if( observation.isMarioOnGround() ){ if( landscape[11+ 2 ][11+ -2 ] != 20 ){ if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ 0 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } action[ Mario.KEY_RIGHT ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ if( landscape[11+ -2 ][11+ 2 ] != -11 ){ if( observation.mayMarioJump() ){ if( enemies[11+ -2 ][11+ -1 ] != Sprite.KIND_GOOMBA_WINGED ){ if( landscape[11+ 1 ][11+ -1 ] != 20 ){ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ action[ Mario.KEY_DOWN ] = false ;
 } } }else{ action[ Mario.KEY_LEFT ] = false ;
 } } if( landscape[11+ -2 ][11+ 2 ] != 16 ){ if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( landscape[11+ -1 ][11+ 0 ] != -10 ){ if( landscape[11+ 1 ][11+ 0 ] != 16 ){ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 } } } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ action[ Mario.KEY_LEFT ] = true ;
