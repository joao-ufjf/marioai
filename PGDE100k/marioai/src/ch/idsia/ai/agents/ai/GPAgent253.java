 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent253 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( landscape[11+ -1 ][11+ -1 ] != 1 ){ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ if( landscape[11+ -1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 action[ Mario.KEY_DOWN ] = true ;
 } } if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ -1 ] != 1 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
