...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( landscape[11+ 2 ][11+ 1 ] != 16 ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } }else{ if( landscape[11+ -1 ][11+ -2 ] != 20 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_SPEED ] = true ;
 }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 } } } } if( landscape[11+ -2 ][11+ -2 ] != -10 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } } }else{ if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_LEFT ] = true ;
