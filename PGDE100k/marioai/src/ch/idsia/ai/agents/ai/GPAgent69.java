 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent69 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ -1 ][11+ 2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_SPEED ] = true ;
 } if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } if( observation.isMarioOnGround() ){ if( landscape[11+ -1 ][11+ 1 ] != 21 ){ action[ Mario.KEY_LEFT ] = false ;
 } }else{ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( enemies[11+ -2 ][11+ -2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ if( enemies[11+ 1 ][11+ 2 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ -1 ][11+ -1 ] != 0 ){ action[ Mario.KEY_DOWN ] = false ;
 } } } } } }else{ if( landscape[11+ 0 ][11+ 1 ] != -11 ){ if( enemies[11+ 1 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( landscape[11+ 1 ][11+ 1 ] != -11 ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } } }else{ if( observation.mayMarioJump() ){ if( landscape[11+ -2 ][11+ 2 ] != 21 ){ action[ Mario.KEY_LEFT ] = true ;
 } } } } 	
	//Begin of BGPAgent
	return action;
    }
