...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ action[ Mario.KEY_SPEED ] = false ;
 action[ Mario.KEY_SPEED ] = false ;
 } action[ Mario.KEY_RIGHT ] = false ;
 } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 } } } } }else{ if( landscape[11+ 0 ][11+ -1 ] != 20 ){ if( observation.mayMarioJump() ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ if( landscape[11+ 1 ][11+ 0 ] != -11 ){ if( enemies[11+ -1 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } } } } } } 	
	//Begin of BGPAgent
	return action;
    }
