 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent434 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ -1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_RIGHT ] = true ;
 ...
 } } } }else{ if( landscape[11+ 0 ][11+ 1 ] != 1 ){ if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ action[ Mario.KEY_DOWN ] = false ;
 action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_SPEED ] = true ;
 } }else{ action[ Mario.KEY_DOWN ] = false ;
