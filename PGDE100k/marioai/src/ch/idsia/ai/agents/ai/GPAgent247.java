 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent247 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 0 ][11+ -1 ] != 1 ){ if( observation.isMarioOnGround() ){ if( landscape[11+ -1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_DOWN ] = true ;
 ...
 action[ Mario.KEY_LEFT ] = true ;
 }else{ if( enemies[11+ 0 ][11+ -1 ] != 1 ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
