...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_SPIKY ){ if( enemies[11+ 2 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
 } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_LEFT ] = true ;
 }else{ if( landscape[11+ 2 ][11+ 0 ] != 21 ){ action[ Mario.KEY_JUMP ] = true ;
 } if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } } } } }else{ if( landscape[11+ 0 ][11+ -1 ] != 0 ){ if( enemies[11+ -2 ][11+ 1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_SPEED ] = false ;
 if( landscape[11+ 1 ][11+ -1 ] != 20 ){ if( landscape[11+ 1 ][11+ 2 ] != -11 ){ action[ Mario.KEY_JUMP ] = true ;
