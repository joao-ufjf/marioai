...
//                        show(cur_char);
 ...
 }else{ if( landscape[11+ -2 ][11+ -1 ] != 20 ){ if( landscape[11+ 2 ][11+ 0 ] != -10 ){ action[ Mario.KEY_JUMP ] = false ;
 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } } } if( enemies[11+ 0 ][11+ 2 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_RIGHT ] = true ;
 } } } 	
	//Begin of BGPAgent
	return action;
    }
