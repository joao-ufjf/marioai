...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } }else{ if( enemies[11+ -2 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ if( landscape[11+ -1 ][11+ 0 ] != 0 ){ action[ Mario.KEY_LEFT ] = true ;
 } if( landscape[11+ 1 ][11+ 2 ] != 16 ){ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } action[ Mario.KEY_JUMP ] = false ;
 action[ Mario.KEY_RIGHT ] = true ;
 } action[ Mario.KEY_LEFT ] = false ;
 }else{ if( enemies[11+ 1 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_JUMP ] = false ;
