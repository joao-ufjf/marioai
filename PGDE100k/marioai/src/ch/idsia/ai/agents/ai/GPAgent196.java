...
//                        show(cur_char);
 ...
 if( observation.mayMarioJump() ){ if( landscape[11+ -2 ][11+ 1 ] != 20 ){ if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = false ;
 } } }else{ action[ Mario.KEY_RIGHT ] = false ;
 action[ Mario.KEY_SPEED ] = false ;
 } action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ -1 ] != Sprite.KIND_GOOMBA_WINGED ){ if( landscape[11+ 0 ][11+ -1 ] != -10 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ if( landscape[11+ -2 ][11+ 0 ] != -10 ){ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_SPEED ] = false ;
 action[ Mario.KEY_SPEED ] = false ;
 } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_RED_KOOPA ){ action[ Mario.KEY_RIGHT ] = true ;
