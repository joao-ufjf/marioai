 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent769 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 1 ][11+ 0 ] != 1 ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = true ;
 ...
 } } if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ 0 ] != 1 ){ if( enemies[11+ -1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_RIGHT ] = false ;
