 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent427 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 1 ][11+ 0 ] != 1 ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 } }else{ if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ action[ Mario.KEY_JUMP ] = true ;
 action[ Mario.KEY_RIGHT ] = false ;
 } } } 	
