 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent867 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ -1 ][11+ 0 ] != 1 ){ if( landscape[11+ 1 ][11+ -1 ] != 1 ){ if( enemies[11+ 1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_LEFT ] = false ;
 ...
 } } } if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
