...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_GOOMBA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } } } action[ Mario.KEY_DOWN ] = true ;
 if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 0 ] != Sprite.KIND_GOOMBA_WINGED ){ if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ -1 ] != Sprite.KIND_RED_KOOPA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } }else{ if( landscape[11+ 1 ][11+ 0 ] != 16 ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } } } } if( landscape[11+ 1 ][11+ 1 ] != 20 ){ if( landscape[11+ 0 ][11+ 0 ] != 20 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = true ;
 } } action[ Mario.KEY_DOWN ] = true ;
 }else{ if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_DOWN ] = true ;
 if( enemies[11+ 0 ][11+ -1 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
