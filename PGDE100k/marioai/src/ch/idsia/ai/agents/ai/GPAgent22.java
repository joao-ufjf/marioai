...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 1 ][11+ -2 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( enemies[11+ 1 ][11+ 2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = true ;
 } }else{ if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_RIGHT ] = true ;
 } } }else{ if( enemies[11+ -1 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_SPEED ] = true ;
