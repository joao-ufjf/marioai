...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( enemies[11+ -1 ][11+ 2 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } }else{ if( enemies[11+ -2 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ action[ Mario.KEY_LEFT ] = false ;
