...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ -2 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 if( landscape[11+ 2 ][11+ -1 ] != 21 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 } }else{ if( enemies[11+ -1 ][11+ -2 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_DOWN ] = true ;
 if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_LEFT ] = false ;
