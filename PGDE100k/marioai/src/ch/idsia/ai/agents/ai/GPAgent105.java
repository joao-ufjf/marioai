...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 1 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ if( landscape[11+ 0 ][11+ -1 ] != 20 ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_BULLET_BILL ){ if( landscape[11+ -1 ][11+ 1 ] != -10 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ action[ Mario.KEY_JUMP ] = false ;
 action[ Mario.KEY_RIGHT ] = false ;
 } action[ Mario.KEY_RIGHT ] = false ;
 } }else{ if( observation.mayMarioJump() ){ if( landscape[11+ -2 ][11+ 0 ] != 16 ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = true ;
