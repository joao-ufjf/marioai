...
//        cin.get();
    ...
    double** rk4 = RKEvaluateVariable(s1, dataset, data->variables + data->prediction, dataset[1][0]-dataset[0][0], tamDataset);
...
double ** RK4LSDParser::RKEvaluateVariable(Subject* s, double** matrix, int vars, double h, int iterations) {
    ...
    for(int i = 0; i < iterations - 1; i++) {
        ...
        for(int j = 1; j < vars; j++)
            increments[j] = 0;
        for(int j = 1; j < vars; j++) // s: modelos j:especific model r[i]:variables increments:vector
            k[j][0] = h * AuxRKEvaluate(dynamic_cast<RK4LSIndividuo*>(s), j, r[i], increments);
        ...
        for(int j = 1; j < vars; j++)
            increments[j] = k[j][0]/2.0;
        for(int j = 1; j < vars; j++)
            k[j][1] = h * AuxRKEvaluate(dynamic_cast<RK4LSIndividuo*>(s), j, r[i], increments);
        ...
        for(int j = 1; j < vars; j++)
            increments[j] = k[j][1]/2.0;
        for(int j = 1; j < vars; j++)
            k[j][2] = h * AuxRKEvaluate(dynamic_cast<RK4LSIndividuo*>(s), j, r[i], increments);
        ...
        for(int j = 1; j < vars; j++)
            increments[j] = k[j][2];
        for(int j = 1; j < vars; j++)
            k[j][3] = h * AuxRKEvaluate(dynamic_cast<RK4LSIndividuo*>(s), j, r[i], increments);
