 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent34 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( landscape[11+ 2 ][11+ 0 ] != 21 ){ action[ Mario.KEY_JUMP ] = true ;
 } if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( landscape[11+ 0 ][11+ 2 ] != 21 ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 action[ Mario.KEY_DOWN ] = false ;
 } if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_SPEED ] = false ;
 } }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 } } } if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_DOWN ] = false ;
 if( enemies[11+ 0 ][11+ -1 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_JUMP ] = false ;
 } } } if( landscape[11+ -2 ][11+ -3 ] != -11 ){ if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ 3 ] != Sprite.KIND_GREEN_KOOPA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } }else{ if( landscape[11+ 1 ][11+ -2 ] != 20 ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 action[ Mario.KEY_LEFT ] = true ;
 } } if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( enemies[11+ 3 ][11+ -2 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ -2 ] != 21 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } } } } if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ 1 ][11+ 2 ] != -10 ){ if( enemies[11+ 2 ][11+ 3 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_LEFT ] = false ;
 } if( enemies[11+ 1 ][11+ 1 ] != Sprite.KIND_RED_KOOPA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = false ;
 } }else{ action[ Mario.KEY_RIGHT ] = false ;
 } } }else{ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_LEFT ] = false ;
 }else{ if( landscape[11+ -2 ][11+ -1 ] != -10 ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } } } } } 	
	//Begin of BGPAgent
	return action;
