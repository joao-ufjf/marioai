 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent265 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ action[ Mario.KEY_DOWN ] = true ;
 ...
 action[ Mario.KEY_SPEED ] = false ;
 action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ -1 ][11+ 0 ] != 1 ){ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_RIGHT ] = false ;
