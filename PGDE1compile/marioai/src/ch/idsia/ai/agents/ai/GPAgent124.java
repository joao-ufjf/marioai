 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent124 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 1 ] != 16 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 } } }else{ if( landscape[11+ 2 ][11+ 0 ] != 0 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } action[ Mario.KEY_JUMP ] = false ;
 } } if( landscape[11+ -2 ][11+ -2 ] != 0 ){ if( enemies[11+ 1 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = false ;
 } } }else{ if( landscape[11+ 1 ][11+ -2 ] != -11 ){ if( landscape[11+ -2 ][11+ -1 ] != 20 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 } action[ Mario.KEY_LEFT ] = false ;
 action[ Mario.KEY_SPEED ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ 0 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } }else{ if( landscape[11+ 2 ][11+ 2 ] != -11 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ action[ Mario.KEY_JUMP ] = false ;
 action[ Mario.KEY_RIGHT ] = true ;
 } } } if( observation.isMarioOnGround() ){ if( landscape[11+ -2 ][11+ -1 ] != 20 ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 } } }else{ if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } } } }else{ if( landscape[11+ -1 ][11+ 0 ] != -11 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_LEFT ] = false ;
