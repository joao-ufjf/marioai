 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent14 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 } }else{ if( enemies[11+ 0 ][11+ 2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } } if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_JUMP ] = false ;
 } } if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( enemies[11+ -1 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ -2 ][11+ 2 ] != -10 ){ action[ Mario.KEY_LEFT ] = false ;
 ...
 action[ Mario.KEY_LEFT ] = true ;
 } } if( enemies[11+ -2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 } } } } if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_BULLET_BILL ){ if( observation.isMarioOnGround() ){ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_RED_KOOPA ){ if( landscape[11+ 1 ][11+ -3 ] != 0 ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } if( landscape[11+ -1 ][11+ 3 ] != 16 ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ if( enemies[11+ -3 ][11+ 1 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = false ;
 action[ Mario.KEY_RIGHT ] = false ;
 }else{ if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } }else{ action[ Mario.KEY_LEFT ] = false ;
 } } }else{ if( observation.mayMarioJump() ){ if( landscape[11+ -3 ][11+ 0 ] != 0 ){ action[ Mario.KEY_SPEED ] = true ;
 } } } 	
	//Begin of BGPAgent
	return action;
    }
