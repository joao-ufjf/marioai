 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent790 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( landscape[11+ -1 ][11+ 1 ] != 1 ){ if( enemies[11+ -1 ][11+ 1 ] != 1 ){ if( landscape[11+ -1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_DOWN ] = false ;
 ...
 } } } } if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 1 ] != 1 ){ if( enemies[11+ -1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_LEFT ] = false ;
 }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = false ;
