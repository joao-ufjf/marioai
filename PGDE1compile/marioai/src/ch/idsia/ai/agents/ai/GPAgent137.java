...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( landscape[11+ 0 ][11+ 1 ] != 20 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } }else{ action[ Mario.KEY_SPEED ] = true ;
 } if( landscape[11+ 0 ][11+ 0 ] != 0 ){ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_JUMP ] = true ;
 } } if( enemies[11+ -2 ][11+ -1 ] != Sprite.KIND_SPIKY_WINGED ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } }else{ action[ Mario.KEY_SPEED ] = true ;
 ...
 } if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( landscape[11+ 1 ][11+ 2 ] != 0 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } } }else{ if( landscape[11+ 2 ][11+ 2 ] != -11 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 } } } } if( landscape[11+ -1 ][11+ 1 ] != 0 ){ if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
