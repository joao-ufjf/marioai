...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 1 ] != 16 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 ...
 } if( enemies[11+ -1 ][11+ 2 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 } } }else{ if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ if( enemies[11+ -2 ][11+ -1 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_DOWN ] = false ;
