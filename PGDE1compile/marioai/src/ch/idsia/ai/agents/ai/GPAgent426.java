 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent426 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ -1 ][11+ 1 ] != 1 ){ if( landscape[11+ -1 ][11+ -1 ] != 1 ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } }else{ if( enemies[11+ 1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } } if( enemies[11+ 0 ][11+ 1 ] != 1 ){ if( landscape[11+ -1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_DOWN ] = true ;
