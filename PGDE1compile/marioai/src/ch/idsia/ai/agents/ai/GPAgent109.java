...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } } }else{ if( landscape[11+ -1 ][11+ 0 ] != 20 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } action[ Mario.KEY_SPEED ] = true ;
 } } action[ Mario.KEY_LEFT ] = true ;
 }else{ if( landscape[11+ -2 ][11+ -2 ] != 0 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 } }else{ if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
