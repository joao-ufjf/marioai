...
//                        show(cur_char);
 ...
 } if( enemies[11+ -2 ][11+ -1 ] != Sprite.KIND_SPIKY_WINGED ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 ...
 } } } } if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ -2 ][11+ 1 ] != 20 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_SPEED ] = true ;
 } } } 	
	//Begin of BGPAgent
	return action;
