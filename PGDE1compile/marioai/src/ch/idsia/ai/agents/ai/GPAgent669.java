 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent669 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 0 ] != 1 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 } } } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 1 ] != 1 ){ if( landscape[11+ -1 ][11+ 0 ] != 1 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } }else{ action[ Mario.KEY_DOWN ] = false ;
