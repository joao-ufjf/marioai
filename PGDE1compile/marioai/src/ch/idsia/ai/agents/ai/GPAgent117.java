...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ -1 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_SPEED ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = false ;
 } } }else{ if( landscape[11+ -1 ][11+ 0 ] != 20 ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_SPEED ] = true ;
 ...
 } }else{ if( observation.isMarioOnGround() ){ if( landscape[11+ 2 ][11+ 0 ] != 20 ){ action[ Mario.KEY_DOWN ] = true ;
 } }else{ if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_SPEED ] = false ;
 } action[ Mario.KEY_RIGHT ] = false ;
 action[ Mario.KEY_LEFT ] = false ;
 } if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_GOOMBA_WINGED ){ if( observation.isMarioOnGround() ){ if( landscape[11+ 2 ][11+ 2 ] != 0 ){ action[ Mario.KEY_JUMP ] = true ;
 action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 if( enemies[11+ 1 ][11+ -2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = true ;
 } if( landscape[11+ 0 ][11+ -2 ] != -10 ){ action[ Mario.KEY_RIGHT ] = true ;
 } } } } 	
	//Begin of BGPAgent
	return action;
    }
