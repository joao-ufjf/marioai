 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent39 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_SPEED ] = false ;
 } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } } } } if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ 2 ] != 20 ){ action[ Mario.KEY_JUMP ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( enemies[11+ 3 ][11+ -1 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_SPEED ] = true ;
 } } } action[ Mario.KEY_LEFT ] = false ;
 if( landscape[11+ -2 ][11+ -3 ] != -11 ){ if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ 2 ] != -10 ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 action[ Mario.KEY_LEFT ] = true ;
 } } if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( enemies[11+ 3 ][11+ 3 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ -2 ] != 0 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } } } if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ 1 ][11+ 2 ] != -10 ){ if( enemies[11+ 2 ][11+ 3 ] != Sprite.KIND_RED_KOOPA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } } } }else{ if( landscape[11+ -3 ][11+ 2 ] != 16 ){ action[ Mario.KEY_DOWN ] = true ;
