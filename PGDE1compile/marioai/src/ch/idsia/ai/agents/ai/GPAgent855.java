 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent855 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_JUMP ] = true ;
 } if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } }else{ action[ Mario.KEY_SPEED ] = false ;
 action[ Mario.KEY_SPEED ] = true ;
 } if( enemies[11+ -1 ][11+ 1 ] != 1 ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
