 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent579 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ -1 ] != 1 ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 }else{ if( enemies[11+ 0 ][11+ 1 ] != 1 ){ if( landscape[11+ 1 ][11+ -1 ] != 1 ){ action[ Mario.KEY_JUMP ] = false ;
