 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent814 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( landscape[11+ 1 ][11+ 1 ] != 1 ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 } }else{ if( landscape[11+ 1 ][11+ -1 ] != 1 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_SPEED ] = true ;
 } } } 	
