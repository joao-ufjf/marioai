 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent181 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 } } }else{ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ -1 ] != Sprite.KIND_BULLET_BILL ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } } }else{ if( landscape[11+ 0 ][11+ -2 ] != -11 ){ if( enemies[11+ 1 ][11+ -1 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_SPEED ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = false ;
 action[ Mario.KEY_JUMP ] = true ;
 } } } }else{ if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( enemies[11+ 2 ][11+ -2 ] != Sprite.KIND_SPIKY_WINGED ){ if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ 0 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_RIGHT ] = true ;
 ...
 } } }else{ if( enemies[11+ 0 ][11+ 0 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = true ;
 } } } } } } 	
	//Begin of BGPAgent
	return action;
    }
