...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ -1 ] != 1 ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 } if( enemies[11+ 1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = false ;
 } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ action[ Mario.KEY_JUMP ] = true ;
 } } } } }else{ if( landscape[11+ 0 ][11+ -1 ] != 1 ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
 } }else{ if( landscape[11+ 1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } } if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ -1 ] != 1 ){ action[ Mario.KEY_DOWN ] = true ;
