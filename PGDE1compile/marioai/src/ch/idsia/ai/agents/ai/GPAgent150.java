 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent150 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 } if( landscape[11+ 0 ][11+ 3 ] != 21 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 ...
 } }else{ if( enemies[11+ 0 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } } } if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 2 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } if( observation.isMarioOnGround() ){ if( enemies[11+ 3 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( enemies[11+ 3 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( enemies[11+ -3 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ action[ Mario.KEY_JUMP ] = true ;
 } } } } if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 } if( landscape[11+ 0 ][11+ 3 ] != -10 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( landscape[11+ 0 ][11+ -2 ] != -11 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } } if( enemies[11+ -1 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_SPEED ] = true ;
 ...
 } } } if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 2 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } 	
	//Begin of BGPAgent
