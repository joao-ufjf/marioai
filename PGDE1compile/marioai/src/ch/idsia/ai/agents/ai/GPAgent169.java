 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent169 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 3 ][11+ -2 ] != Sprite.KIND_RED_KOOPA ){ if( landscape[11+ 2 ][11+ -2 ] != -10 ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 } }else{ if( enemies[11+ 0 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } } } if( landscape[11+ 3 ][11+ 1 ] != 21 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 ...
 } } } } if( landscape[11+ -1 ][11+ -3 ] != 16 ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
