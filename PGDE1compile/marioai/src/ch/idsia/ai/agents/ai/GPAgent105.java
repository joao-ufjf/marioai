...
//                        show(cur_char);
 ...
 } } } } if( landscape[11+ -1 ][11+ 2 ] != 0 ){ if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ if( enemies[11+ -1 ][11+ 2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = false ;
 action[ Mario.KEY_DOWN ] = false ;
 }else{ if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = false ;
 } } } action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } } 	
