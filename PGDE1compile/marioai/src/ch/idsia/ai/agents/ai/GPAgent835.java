 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent835 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ -1 ][11+ 1 ] != 1 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = true ;
 action[ Mario.KEY_RIGHT ] = true ;
 ...
 }else{ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
