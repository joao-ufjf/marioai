...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 1 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( enemies[11+ 2 ][11+ -3 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ 1 ][11+ 0 ] != 16 ){ if( enemies[11+ 1 ][11+ -1 ] != Sprite.KIND_GOOMBA_WINGED ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } }else{ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_JUMP ] = false ;
 } } } if( landscape[11+ 1 ][11+ -2 ] != 20 ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = false ;
 action[ Mario.KEY_JUMP ] = false ;
 } action[ Mario.KEY_JUMP ] = true ;
 } action[ Mario.KEY_LEFT ] = false ;
 }else{ if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_JUMP ] = true ;
 } } 	
	//Begin of BGPAgent
	return action;
    }
