 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent513 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 0 ][11+ 0 ] != 1 ){ if( landscape[11+ 1 ][11+ 1 ] != 1 ){ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 1 ] != 1 ){ action[ Mario.KEY_DOWN ] = false ;
 if( landscape[11+ 1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_LEFT ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } }else{ if( enemies[11+ -1 ][11+ 0 ] != 1 ){ if( landscape[11+ -1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_JUMP ] = true ;
