 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent24 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 } }else{ if( landscape[11+ 0 ][11+ 2 ] != -10 ){ action[ Mario.KEY_SPEED ] = true ;
 ...
 } if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( enemies[11+ 3 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_SPEED ] = true ;
 } } } if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ 3 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 action[ Mario.KEY_SPEED ] = true ;
 }else{ if( enemies[11+ -3 ][11+ 3 ] != Sprite.KIND_RED_KOOPA ){ action[ Mario.KEY_DOWN ] = true ;
 ...
 } } } } if( enemies[11+ -2 ][11+ 0 ] != Sprite.KIND_GOOMBA_WINGED ){ if( enemies[11+ 3 ][11+ 3 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } } } } if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_GOOMBA ){ if( observation.isMarioOnGround() ){ if( enemies[11+ 3 ][11+ -1 ] != Sprite.KIND_GOOMBA_WINGED ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_RED_KOOPA ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } } } }else{ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_LEFT ] = false ;
 } } } } 	
