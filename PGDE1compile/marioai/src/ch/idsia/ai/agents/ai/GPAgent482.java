 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent482 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( landscape[11+ 0 ][11+ -1 ] != 1 ){ if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ 0 ] != 1 ){ action[ Mario.KEY_SPEED ] = true ;
 ...
 if( enemies[11+ 0 ][11+ 0 ] != 1 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
