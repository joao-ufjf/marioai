 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent81 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 } }else{ if( landscape[11+ 0 ][11+ 2 ] != -10 ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } } if( observation.mayMarioJump() ){ if( landscape[11+ -2 ][11+ -1 ] != -11 ){ action[ Mario.KEY_JUMP ] = false ;
 ...
 } if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( enemies[11+ -3 ][11+ -3 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_LEFT ] = false ;
 } action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_JUMP ] = false ;
 } } if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( enemies[11+ -2 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 action[ Mario.KEY_DOWN ] = true ;
 }else{ if( landscape[11+ -2 ][11+ -3 ] != 21 ){ action[ Mario.KEY_RIGHT ] = false ;
 } } } } } if( landscape[11+ -2 ][11+ -3 ] != 0 ){ if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ 3 ] != -10 ){ action[ Mario.KEY_JUMP ] = true ;
 } if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } }else{ if( enemies[11+ 2 ][11+ 3 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_JUMP ] = false ;
 } } if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 } } } } if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_BULLET_BILL ){ if( landscape[11+ 1 ][11+ 2 ] != 20 ){ if( observation.isMarioOnGround() ){ if( landscape[11+ -1 ][11+ 0 ] != -11 ){ action[ Mario.KEY_LEFT ] = true ;
 } if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } } }else{ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ 3 ] != -11 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = true ;
