...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ 2 ] != Sprite.KIND_SPIKY ){ if( observation.isMarioOnGround() ){ if( landscape[11+ 1 ][11+ -1 ] != 16 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } } if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
 } if( landscape[11+ -2 ][11+ -2 ] != 0 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } } } }else{ if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_LEFT ] = true ;
