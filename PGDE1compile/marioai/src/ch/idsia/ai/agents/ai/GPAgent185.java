 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent185 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( landscape[11+ 3 ][11+ 1 ] != 16 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = true ;
 ...
 } } if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ -1 ][11+ -3 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
 action[ Mario.KEY_DOWN ] = true ;
 if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = false ;
 } }else{ action[ Mario.KEY_DOWN ] = false ;
 } if( landscape[11+ -2 ][11+ -3 ] != -11 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = true ;
 ...
 action[ Mario.KEY_LEFT ] = true ;
 } } if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = false ;
 } if( landscape[11+ 3 ][11+ -3 ] != 20 ){ action[ Mario.KEY_LEFT ] = true ;
 } if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = true ;
 } } if( observation.isMarioOnGround() ){ if( landscape[11+ 1 ][11+ 2 ] != -10 ){ if( enemies[11+ -3 ][11+ 3 ] != Sprite.KIND_RED_KOOPA ){ if( landscape[11+ -2 ][11+ 1 ] != 20 ){ action[ Mario.KEY_JUMP ] = true ;
