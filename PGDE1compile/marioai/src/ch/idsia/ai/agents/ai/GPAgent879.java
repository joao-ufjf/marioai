 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent879 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( enemies[11+ 0 ][11+ 1 ] != 1 ){ if( enemies[11+ -1 ][11+ -1 ] != 1 ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 } if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = false ;
 } action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_SPEED ] = true ;
