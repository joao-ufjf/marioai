...
//                        show(cur_char);
 ...
 } }else{ if( landscape[11+ 3 ][11+ 1 ] != 0 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } } } if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_SPIKY_WINGED ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_SPEED ] = false ;
 }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } } } } } if( landscape[11+ -2 ][11+ -2 ] != -11 ){ if( observation.mayMarioJump() ){ if( landscape[11+ -3 ][11+ 0 ] != 0 ){ action[ Mario.KEY_SPEED ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = false ;
 } action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_LEFT ] = true ;
 } } if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = false ;
 } } } if( enemies[11+ 2 ][11+ -1 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ -3 ][11+ 2 ] != 20 ){ action[ Mario.KEY_SPEED ] = true ;
 } action[ Mario.KEY_JUMP ] = false ;
 }else{ if( enemies[11+ 1 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 } } } 	
	//Begin of BGPAgent
	return action;
    }
