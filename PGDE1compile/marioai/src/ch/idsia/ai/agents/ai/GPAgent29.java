 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent29 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( landscape[11+ -1 ][11+ 0 ] != 0 ){ action[ Mario.KEY_JUMP ] = true ;
 ...
 } if( landscape[11+ 0 ][11+ 3 ] != 16 ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } } if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 0 ] != 20 ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } }else{ if( enemies[11+ -3 ][11+ 0 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_DOWN ] = true ;
 } } action[ Mario.KEY_RIGHT ] = true ;
 ...
 action[ Mario.KEY_LEFT ] = true ;
 } } if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = false ;
 } } } if( landscape[11+ -3 ][11+ 3 ] != 16 ){ if( landscape[11+ 1 ][11+ 2 ] != -10 ){ if( enemies[11+ -3 ][11+ 3 ] != Sprite.KIND_RED_KOOPA ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = false ;
 } } } }else{ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ if( enemies[11+ -1 ][11+ 3 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_DOWN ] = false ;
