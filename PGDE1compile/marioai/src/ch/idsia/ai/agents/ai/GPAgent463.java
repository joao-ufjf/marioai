 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent463 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ 1 ] != 1 ){ if( enemies[11+ -1 ][11+ 0 ] != 1 ){ action[ Mario.KEY_RIGHT ] = true ;
 ...
 } } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 0 ][11+ 0 ] != 1 ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
