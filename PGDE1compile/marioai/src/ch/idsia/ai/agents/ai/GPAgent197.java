...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( enemies[11+ -2 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 }else{ if( landscape[11+ -2 ][11+ -2 ] != 20 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
