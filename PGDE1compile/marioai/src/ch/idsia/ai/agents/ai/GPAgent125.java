 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent125 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
 ...
 } }else{ if( landscape[11+ 0 ][11+ -2 ] != -11 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 } } } if( enemies[11+ -1 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_SPEED ] = true ;
 ...
 } }else{ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } } if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_JUMP ] = false ;
 } } if( observation.isMarioOnGround() ){ if( enemies[11+ 3 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( enemies[11+ -1 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ -2 ][11+ 2 ] != -10 ){ action[ Mario.KEY_LEFT ] = false ;
 action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } } } } if( landscape[11+ -3 ][11+ -2 ] != -11 ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = true ;
 } action[ Mario.KEY_DOWN ] = true ;
 action[ Mario.KEY_LEFT ] = true ;
 } } if( observation.mayMarioJump() ){ if( landscape[11+ 0 ][11+ -1 ] != -11 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } }else{ if( enemies[11+ -2 ][11+ -2 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } } if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_BULLET_BILL ){ if( landscape[11+ 1 ][11+ 1 ] != -10 ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ 0 ][11+ 3 ] != Sprite.KIND_SPIKY_WINGED ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } } } }else{ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } if( enemies[11+ -3 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ action[ Mario.KEY_SPEED ] = false ;
 } } } } 	
	//Begin of BGPAgent
	return action;
    }
