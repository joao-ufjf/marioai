...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 1 ] != 16 ){ if( landscape[11+ 1 ][11+ 0 ] != -11 ){ action[ Mario.KEY_LEFT ] = true ;
 ...
 } if( enemies[11+ 1 ][11+ 1 ] != Sprite.KIND_BULLET_BILL ){ if( observation.mayMarioJump() ){ if( enemies[11+ 2 ][11+ 1 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_SPEED ] = false ;
