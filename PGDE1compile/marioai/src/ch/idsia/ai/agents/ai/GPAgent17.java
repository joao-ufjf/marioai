 ...
 * Package: ch.idsia.ai.agents.ai;
 */
public class GPAgent17 extends BasicAIAgent implements Agent
{
...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( landscape[11+ 2 ][11+ 0 ] != -10 ){ action[ Mario.KEY_RIGHT ] = true ;
 } if( observation.isMarioOnGround() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( landscape[11+ 0 ][11+ 2 ] != 20 ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_SPEED ] = false ;
 ...
 } if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = true ;
 } } if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( enemies[11+ -3 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ action[ Mario.KEY_LEFT ] = false ;
 } } } if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ if( landscape[11+ -2 ][11+ -3 ] != -10 ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } } } } } if( landscape[11+ -2 ][11+ -3 ] != -11 ){ if( observation.mayMarioJump() ){ if( landscape[11+ 3 ][11+ 3 ] != -10 ){ action[ Mario.KEY_JUMP ] = true ;
 } if( observation.mayMarioJump() ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ if( enemies[11+ -3 ][11+ 2 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } } } if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ if( enemies[11+ 3 ][11+ 3 ] != Sprite.KIND_GREEN_KOOPA_WINGED ){ if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 2 ] != 21 ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } } } } if( enemies[11+ 1 ][11+ 3 ] != Sprite.KIND_GOOMBA ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ 0 ][11+ 1 ] != Sprite.KIND_RED_KOOPA ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } } } }else{ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( landscape[11+ 0 ][11+ -2 ] != -11 ){ action[ Mario.KEY_DOWN ] = true ;
