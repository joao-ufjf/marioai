...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA ){ if( observation.mayMarioJump() ){ if( landscape[11+ 1 ][11+ 1 ] != 16 ){ if( landscape[11+ 1 ][11+ 0 ] != -11 ){ action[ Mario.KEY_LEFT ] = true ;
 } }else{ action[ Mario.KEY_JUMP ] = false ;
 if( landscape[11+ -1 ][11+ -1 ] != 0 ){ action[ Mario.KEY_SPEED ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } } }else{ action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_LEFT ] = true ;
 } if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_LEFT ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } } action[ Mario.KEY_LEFT ] = false ;
 action[ Mario.KEY_LEFT ] = true ;
 }else{ if( observation.isMarioOnGround() ){ if( landscape[11+ 1 ][11+ -2 ] != 16 ){ action[ Mario.KEY_LEFT ] = false ;
 } }else{ if( landscape[11+ -1 ][11+ 2 ] != 16 ){ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } } action[ Mario.KEY_RIGHT ] = true ;
 action[ Mario.KEY_DOWN ] = false ;
 } if( enemies[11+ -1 ][11+ 1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( observation.isMarioOnGround() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ if( observation.mayMarioJump() ){ if( enemies[11+ -2 ][11+ 1 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_DOWN ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } } } }else{ if( observation.isMarioOnGround() ){ if( landscape[11+ 0 ][11+ 1 ] != 0 ){ action[ Mario.KEY_RIGHT ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } action[ Mario.KEY_LEFT ] = false ;
