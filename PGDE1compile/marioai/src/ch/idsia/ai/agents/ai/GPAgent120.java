...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ -1 ][11+ 2 ] != Sprite.KIND_RED_KOOPA ){ if( landscape[11+ 1 ][11+ 0 ] != 16 ){ if( landscape[11+ 1 ][11+ 0 ] != 20 ){ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( landscape[11+ 0 ][11+ 0 ] != 21 ){ if( enemies[11+ 0 ][11+ 0 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_RIGHT ] = true ;
 } } } }else{ action[ Mario.KEY_JUMP ] = true ;
 if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = true ;
 } if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ -2 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_JUMP ] = false ;
 } }else{ action[ Mario.KEY_RIGHT ] = false ;
 action[ Mario.KEY_JUMP ] = false ;
 } } action[ Mario.KEY_SPEED ] = true ;
 action[ Mario.KEY_LEFT ] = true ;
 }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_LEFT ] = false ;
 } if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ 1 ] != Sprite.KIND_GREEN_KOOPA ){ if( landscape[11+ 1 ][11+ -1 ] != 21 ){ action[ Mario.KEY_DOWN ] = true ;
 } }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_SPEED ] = true ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } } } if( enemies[11+ -1 ][11+ 0 ] != Sprite.KIND_RED_KOOPA ){ if( observation.mayMarioJump() ){ if( enemies[11+ -2 ][11+ 1 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = false ;
 } } }else{ if( observation.mayMarioJump() ){ if( enemies[11+ 1 ][11+ -2 ] != Sprite.KIND_GOOMBA ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } } } }else{ if( enemies[11+ -2 ][11+ 2 ] != Sprite.KIND_GREEN_KOOPA ){ action[ Mario.KEY_LEFT ] = true ;
