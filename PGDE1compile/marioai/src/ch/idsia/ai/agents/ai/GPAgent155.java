...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( observation.mayMarioJump() ){ action[ Mario.KEY_DOWN ] = true ;
 ...
 } } } if( observation.isMarioOnGround() ){ if( enemies[11+ 0 ][11+ -3 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( observation.isMarioOnGround() ){ if( landscape[11+ 0 ][11+ -3 ] != 16 ){ action[ Mario.KEY_RIGHT ] = true ;
