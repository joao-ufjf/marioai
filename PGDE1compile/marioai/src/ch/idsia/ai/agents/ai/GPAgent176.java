...
//                        show(cur_char);
	...
	byte[][] landscape = observation.getLevelSceneObservation();// default: ZLevelScene = 1  
	 ...
	 if( observation.mayMarioJump() ){ if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( observation.mayMarioJump() ){ if( landscape[11+ -1 ][11+ 0 ] != -10 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
 } }else{ if( observation.isMarioOnGround() ){ if( enemies[11+ 1 ][11+ 0 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_DOWN ] = false ;
 }else{ action[ Mario.KEY_RIGHT ] = true ;
 } action[ Mario.KEY_SPEED ] = false ;
 } } if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ -2 ] != Sprite.KIND_BULLET_BILL ){ action[ Mario.KEY_SPEED ] = false ;
 } if( landscape[11+ -2 ][11+ -2 ] != 0 ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } } } }else{ if( observation.isMarioOnGround() ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ if( enemies[11+ -1 ][11+ -1 ] != Sprite.KIND_RED_KOOPA_WINGED ){ if( enemies[11+ 2 ][11+ 0 ] != Sprite.KIND_GOOMBA_WINGED ){ action[ Mario.KEY_JUMP ] = true ;
 }else{ action[ Mario.KEY_DOWN ] = true ;
 } } if( landscape[11+ -2 ][11+ -2 ] != 20 ){ action[ Mario.KEY_RIGHT ] = true ;
 } } if( observation.isMarioOnGround() ){ if( observation.isMarioOnGround() ){ if( enemies[11+ -2 ][11+ 0 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_SPEED ] = false ;
 action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_SPEED ] = false ;
 } }else{ action[ Mario.KEY_LEFT ] = true ;
 } }else{ if( observation.isMarioOnGround() ){ if( enemies[11+ 2 ][11+ 2 ] != Sprite.KIND_SPIKY ){ action[ Mario.KEY_RIGHT ] = false ;
 }else{ action[ Mario.KEY_LEFT ] = true ;
 } action[ Mario.KEY_LEFT ] = true ;
 action[ Mario.KEY_JUMP ] = false ;
 }else{ action[ Mario.KEY_JUMP ] = false ;
